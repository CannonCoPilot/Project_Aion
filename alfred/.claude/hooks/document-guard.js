...
function matchGlob(pattern, filePath) {
  ...
  if (pattern.startsWith('/')) {
    // Absolute pattern — anchor to start of path so substring false-positives
    // like /evilprocess.env.HOME/.sops.yaml don't match process.env.HOME/.sops.yaml
    anchor = '^';
  } else if (!pattern.startsWith('.')) {
    regex = '(?:^|/)' + regex;
  }

  return new RegExp(anchor + regex + '$').test(filePath);
}
...
function findMatchingRules(config, relativePath, absolutePath) {
  var matched = [];
  for (var i = 0; i < config.rules.length; i++) {
    // Expand ~/ in rule patterns so config authors can write ~/.sops.yaml naturally
    var pattern = expandTilde(config.rules[i].pattern);
    // Absolute patterns match absolute paths; relative patterns match relative paths.
    // This lets rules protect files outside the project (e.g. ~/.sops.yaml,
    // ~/.secrets/*.env.enc) without accidentally matching unrelated in-project paths.
    var targetPath = pattern.startsWith('/') ? absolutePath : relativePath;
    if (matchGlob(pattern, targetPath)) {
      matched.push(config.rules[i]);
    }
  }
  matched.sort(function(a, b) { return patternSpecificity(expandTilde(b.pattern)) - patternSpecificity(expandTilde(a.pattern)); });
  return matched;
}
...
function checkRelatedFiles(rule) {
  var files = rule.relatedFiles || [];
  if (files.length === 0) return [];
  return [{
    check: 'related_files',
    tier: 'medium',
    message: 'Related file(s) may need updating: ' + files.join(', '),
  }];
}
    ...
    for (var ch = 0; ch < checks.length; ch++) {
      ...
      switch (checks[ch]) {
        ...
        case 'shebang_preservation':
          ...
          break;
        case 'related_files':
          if (toggles.structuralChecks) results = checkRelatedFiles(rule);
          break;
...
async function main() {
  ...
  // NOTE: files outside PROJECT_DIR are NOT automatically skipped. Rules with
  // absolute patterns (e.g. process.env.HOME/.sops.yaml or ~/.secrets/*.env.enc)
  // can protect files anywhere on disk. Relative patterns still only match paths
  // inside PROJECT_DIR because toRelativePath returns the unchanged absolute path
  // for outside-project files, which relative patterns cannot match.
  // See findMatchingRules for the absolute-vs-relative routing logic.

  // Load config
  var config = await loadConfig();
  if (!config) {
    // Fail closed: config load failure is a landmine state. If the hook can't
    // load its rules, we cannot verify what's protected, so we must block.
    // Emergency recovery: set DOCUMENT_GUARD_ENABLED=false env var.
    var envVal = process.env.DOCUMENT_GUARD_ENABLED;
    var envDisabled = envVal !== undefined && (envVal === 'false' || envVal === '0' || envVal === '');
    if (envDisabled) {
      console.log(JSON.stringify({ proceed: true }));
      return;
    }
    console.log(JSON.stringify({
      proceed: false,
      message: 'DOCUMENT GUARD [FAIL-CLOSED]: config load failed — hook cannot verify protection rules. Edit blocked.\n\nRemediation:\n  1. Fix the config at .claude/hooks/document-guard.config.js\n  2. Validate with: node --check .claude/hooks/document-guard.config.js\n  3. Or emergency disable: export DOCUMENT_GUARD_ENABLED=false (then re-enable after fix)',
    }));
    return;
  }
  ...
  // Find matching rules (absolute patterns route to absolutePath, relative to relativePath)
  var rules = findMatchingRules(config, relativePath, absolutePath);
...
main().catch(function(err) {
  ...
  // Fail closed on fatal errors. Emergency recovery:
  ...
  console.log(JSON.stringify({
    proceed: false,
    message: 'DOCUMENT GUARD [FAIL-CLOSED]: fatal error in hook — ' + err.message + '\n\nRemediation:\n  1. Check recent edits to .claude/hooks/document-guard.js or document-guard.config.js\n  2. Or emergency disable: export DOCUMENT_GUARD_ENABLED=false (then re-enable after fix)',
  }));
});
