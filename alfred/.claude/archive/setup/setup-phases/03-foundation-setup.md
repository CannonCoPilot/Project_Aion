...
### 5. Initialize Pulse Task Management

Set up the Pulse workspace for task tracking across sessions:

```bash
# Initialize Pulse in the AIfred directory
cd [AIFRED_HOME]
scripts/setup-pulse.sh

# Configure from template
cp Pulse workspaceconfig.yaml.template Pulse workspaceconfig.yaml
```

**Customize `Pulse workspaceconfig.yaml`** based on user's focus areas from Phase 2:
...
**Verify Pulse is working:**

```bash
# Create a test task
pulse create "AIfred setup complete" -t task -p 3 \
...
# Close the test task
pulse close [id] --reason "Setup verification"
```
...
echo 'source "$HOME/Code/AIfred/scripts/pulse-aliases.sh"' >> ~/.bashrc
```
...
- [ ] Pulse workspace initialized and verified
