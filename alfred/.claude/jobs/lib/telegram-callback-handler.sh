...
#
# Telegram buttons and callbacks have been removed. The dashboard is now
# the primary interaction surface for all notifications and approvals.
# Telegram is a dumb pager for critical alerts only.
#
# This file is kept for reference. The cron job should be removed.
# See: .claude/context/systems/notification-routing-policy.md

echo "DEPRECATED: telegram-callback-handler.sh is no longer active. Dashboard is the primary interaction surface." >&2
exit 0
...
log() { echo "[$(date -u '+%Y-%m-%dT%H:%M:%SZ')] $1" | tee -a "$HANDLER_LOG" 2>/dev/null; }
...
answer_callback_query() {
    ...
    curl -s -X POST "${API_BASE}/answerCallbackQuery" \
        ...
        > /dev/null 2>&1 || log "WARNING: Failed to answer callback query $callback_query_id"
}
...
edit_message_text() {
    ...
    curl -s -X POST "${API_BASE}/editMessageText" \
        ...
        > /dev/null 2>&1 || log "WARNING: Failed to edit message $message_id"
}
...
Approved $task_id $(date '+%H:%M')"
    ...
    # ── QID-based precise routing ──
    # qid:<question_id>:<action> format — look up question directly by ID
    if [[ "$callback_data" == qid:* ]]; then
        local qid_rest="${callback_data#qid:}"
        local question_id="${qid_rest%%:*}"
        local action="${qid_rest#*:}"

        log "QID callback: question_id=$question_id action=$action"

        local qjob
        qjob=$("$MSGBUS" query --id "$question_id" 2>/dev/null | jq -r '.data.job // "unknown"' | head -1)
        ...
        case "$action" in
            approve|deny|skip)
                "$MSGBUS" reply --parent "$question_id" \
                    --type question_answered \
                    --source "telegram:callback" \
                    --data "$(jq -nc --arg answer "$action" --arg job "$qjob" \
...
$(echo "$action" | sed 's/.*/\u&/') $(date '+%H:%M')"
                log "Processed QID: question=$question_id answer=$action job=$qjob"
                ;;
            other)
...
Waiting for text response..."
                ...
                log "QID Awaiting text for: question=$question_id job=$qjob"
                ;;
            *)
                answer_callback_query "$callback_query_id" "Unknown action: $action"
                log "WARNING: Unknown QID action=$action for question=$question_id"
                ;;
        esac
        return
    fi
