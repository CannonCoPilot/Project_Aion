...
├── Initialize Pulse task management
├── Configure CLAUDE.md
...
├── Install core hooks (including pulse-actor)
├── Configure session management
├── Set up Nexus dispatcher (cron)
...
```
AIfred/
├── docker-compose.yml          # Pulse task management
...
│   │   ├── pulse-actor         # Session provenance for Pulse
│   │   └── *.js                   # JS hooks (audit, security, workflow)
│   ├── skills/                     # Workflow skills (8)
│   ├── jobs/                       # Nexus job system
...
│   ├── pulse-aliases.sh           # Pulse shell aliases
...
| `pulse-actor` | Session provenance for Pulse tasks |
...
### Phase 7: Pulse + Nexus -- COMPLETE (v2.4.0, 2026-02-12)
- [x] Pulse task management as required dependency
- [x] Pulse actor for session provenance
- [x] Pulse shell aliases and setup script
- [x] Setup wizard integration (prerequisites + foundation phase)
- [x] Nexus job system (dispatcher + executor)
