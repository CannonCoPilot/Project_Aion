...
function generateSettings(merged) {
  ...
  const settings = {
    ...
    version: '2.2',
    // Shared Archon status line. Absolute path on purpose: $CLAUDE_PROJECT_DIR
    // resolves to alfred/, which has no .claude/scripts of its own, so a
    // relative path would silently render nothing. Must be emitted HERE, not
    // hand-added to settings.json — this file is overwritten wholesale below.
    statusLine: {
      type: 'command',
      command: '/Users/nathanielcannon/Claude/Project_Aion/.claude/scripts/jarvis-statusline-v9.sh'
    },
    hooks: {},
    permissions: {
