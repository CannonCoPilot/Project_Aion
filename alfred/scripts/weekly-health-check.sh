...
check_network() {
    ...
    # Check NAS mount (configure NAS_MOUNT_PATH in .env, e.g. /mnt/synology_nas)
    NAS_MOUNT="${NAS_MOUNT_PATH:-/mnt/nas}"
    NAS_MOUNT_NAME=$(basename "$NAS_MOUNT")
    NAS_MOUNT_COUNT=$(mount | grep -c "$NAS_MOUNT_NAME" 2>/dev/null || echo "0")
    if [[ $NAS_MOUNT_COUNT -gt 0 ]]; then
        ...
        if timeout 5 ls "$NAS_MOUNT" >/dev/null 2>&1; then
            ...
            NAS_FREE=$(df -h "$NAS_MOUNT" 2>/dev/null | tail -1 | awk '{print $4}') || NAS_FREE="unknown"
            pass "NAS mounted ($NAS_MOUNT_COUNT shares, $NAS_FREE free)" "nfs_nas"
        else
            fail "NAS mount is STALE" "nfs_nas" "Mounts exist but not responding - NAS may be down or network issue"
        fi
    else
        # Check why not mounted
        if [[ ! -d "$NAS_MOUNT" ]]; then
            MOUNT_DETAIL="Mount point $NAS_MOUNT does not exist"
        elif grep -q "$NAS_MOUNT_NAME" /etc/fstab 2>/dev/null; then
            # Check if NAS is reachable - extract first IP/hostname from fstab entry
            NAS_IP=$(grep "$NAS_MOUNT_NAME" /etc/fstab 2>/dev/null | head -1 | awk '{print $1}' | cut -d: -f1 | head -1)
        ...
        fail "NAS not mounted" "nfs_nas" "$MOUNT_DETAIL"
    fi
    ...
    # Test a public endpoint with detailed error (configure PUBLIC_HEALTH_URL in .env)
    ...
    if [[ $PUBLIC_EXIT -eq 0 ]] && [[ "$HTTP_CODE" =~ ^(200|301|302|401|403)$ ]]; then
        pass "Public endpoints accessible ($PUBLIC_URL HTTP $HTTP_CODE)" "caddy_public"
    else
        if [[ $PUBLIC_EXIT -eq 6 ]]; then
            fail "Public endpoint failed" "caddy_public" "DNS resolution failed for $PUBLIC_URL"
        ...
        skip "Public endpoint check skipped (PUBLIC_HEALTH_URL not set)" "caddy_public"
    fi
}
