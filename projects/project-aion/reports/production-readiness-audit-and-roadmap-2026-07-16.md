...
| L-2 | **Current JICM checkpoint built from the W5 dev session**, not W0 | `.jicm-last-compression.json` sourced `fbd7528a…jsonl` (this review session) | Do **not** rely on JICM auto-resume for W0 until prep is UUID-scoped (M1). A manual checkpoint is safer. |
