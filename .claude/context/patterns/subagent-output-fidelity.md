...
**Secondary — cross-workspace shadow definitions**: `/Users/nathanielcannon/Claude/Project_Aion-Dev/` is registered as an additionalDirectory in this workspace's Claude Code env. The harness aggregates `.claude/agents/` from ALL listed dirs; name collisions on `code-review` / `code-analyzer` / `code-implementer` / `code-tester` / `project-manager` silently merged with Jarvis-Dev's older Apr-21 broken frontmatter winning the merge. Fixed at Jarvis-Dev commit `6601d6d` on `dev` branch (PUSHED to `CannonCoPilot/Jarvis:dev`).
