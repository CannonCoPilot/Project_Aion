...
The primary (L1) was written conservatively — Rule 3 backed off from ANY input-line text, on the theory Sir might be composing. The backup shell script (L3) was written simply — classify as `active/delegating/paused` with no "user composing" awareness. Result: L1 fired 8 times and sent ZERO full continue-prompts (every state was either active, delegating, or "input has text so back off"). L3 fired ~7 times and sent 5 continue-prompts. The backup became the actor; the primary became the reporting layer. This worked out fine because the "input has text" cases were mostly Sir having just submitted (input line briefly stale before clearing) or stale text from a prior L3 fire. But the design accidentally landed on "primary is conservative, backup is aggressive" — no principled asymmetry.

**Next-time refinement:**
Either (a) make BOTH layers use the same classifier (share the shell script's logic between L1 primary and L3 backup) so the aggressive-vs-conservative split is deliberate, or (b) add a "verify submission by re-capturing" step to both layers so send failures are visible instead of silently piling up in the input buffer. The stale-input-from-prior-fire pathology is a symptom of not verifying submissions.
