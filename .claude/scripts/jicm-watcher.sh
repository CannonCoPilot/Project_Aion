...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --threshold) JICM_TOKEN_THRESHOLD="$2"; shift 2 ;;
        --interval) POLL_INTERVAL="$2"; shift 2 ;;
        -h|--help)
            echo "JICM v7.2 Watcher — Stop-and-Wait Context Management"
            echo ""
...
# JICM Configuration - auto-generated by jicm-watcher.sh v7.2 on startup
...
draw_progress_bar() {
    ...
    if [[ ${LAST_TOKENS:-0} -ge $((JICM_TOKEN_THRESHOLD * 80 / 100)) ]]; then
...
refresh_header() {
    ...
    printf "${C_CYAN}│${C_RESET} Trigger: ${C_YELLOW}%dk tokens${C_RESET}    Session: %-8s  Poll: %s (%ds)" \
        "$((JICM_TOKEN_THRESHOLD / 1000))" "$uptime" "$ts" "$POLL_INTERVAL"
    tput el 2>/dev/null || true
...
refresh_header_legacy() {
    ...
    echo -e "${C_CYAN}║${C_RESET}  Trigger: ${C_YELLOW}$((JICM_TOKEN_THRESHOLD / 1000))k tokens${C_RESET}                                 ${C_CYAN}║${C_RESET}"
...
main() {
    ...
    log INFO "JICM v7.2 Watcher started (token_threshold=${JICM_TOKEN_THRESHOLD}, interval=${POLL_INTERVAL}s)"
    write_state
    ...
    while true; do
        ...
        if [[ "$JICM_STATE" == "WATCHING" ]]; then
            ...
            # Threshold check → start JICM cycle (absolute token count only)
