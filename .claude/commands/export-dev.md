...
1. **Create export directory** (if missing):
   ```bash
   mkdir -p /Users/nathanielcannon/Claude/Project_Aion/.claude/exports/dev
   ```

2. **Generate timestamped filename**:
   ```bash
   EXPORT_FILE="/Users/nathanielcannon/Claude/Project_Aion/.claude/exports/dev/export_dev_chat_$(date +%Y%m%d-%H%M%S).txt"
   ```
...
5. **Prune old exports** — keep the 30 most recent, remove older ones:
   ```bash
   ls -t /Users/nathanielcannon/Claude/Project_Aion/.claude/exports/dev/export_dev_chat_*.txt | tail -n +31 | xargs rm -f 2>/dev/null
   ```
...
Exported dev chat to:
  /Users/nathanielcannon/Claude/Project_Aion/.claude/exports/dev/export_dev_chat_20260212-141530.txt
