...
1. Watcher liveness check:
   ```bash
   WP=$(cat /Users/nathanielcannon/Claude/Jarvis/.claude/context/.jicm-watcher.pid 2>/dev/null)
...
2. Cycle-in-flight check: `ls /Users/nathanielcannon/Claude/Jarvis/.claude/context/.jicm-clear-now.signal 2>/dev/null` — if exists, say "Cycle already in flight." and STOP.
...
6. Check exit code:
   ...
   - **Failure (non-zero)**: say "Prep script failed (exit $?)." and clean up — do NOT write the trigger signal.
     ```bash
     rm -f /Users/nathanielcannon/Claude/Jarvis/.claude/context/.compression-done.signal
     ```
